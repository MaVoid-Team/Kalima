    ...
    <section className="w-full bg-surface-gray py-16 md:py-24">
      <div className="container mx-auto px-4 md:px-8" dir={isRTL ? "rtl" : "ltr"}>
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12">
          <div className={`space-y-4 max-w-2xl ${isRTL ? "text-right" : ""}`}>
          ...
          >
            {t("subjects.viewAll")}
            {isRTL ? (
              <ArrowLeft className="ms-1.5 h-4 w-4 transition-transform group-hover:-translate-x-1" />
            ) : (
              <ArrowRight className="ms-1.5 h-4 w-4 transition-transform group-hover:translate-x-1" />
            )}
          </a>
        </div>
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {subjects.map((subject) => (
            ...
            >
              ...
              {/* Content Half */}
              <div className={`flex flex-col p-8 pt-8 ${isRTL ? "text-right" : "text-left"}`}>
                <div
