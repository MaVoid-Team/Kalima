...
import { User, LogOut, Home, Globe, Moon, Sun, ChevronLeft, Menu, X, Settings, BookOpenCheck } from 'lucide-react';
...
export default function StudentSidebar({ isMobileOpen, setIsMobileOpen }) {
  ...
  useEffect(() => {
    const savedTheme = localStorage.getItem(STUDENT_THEME_STORAGE_KEY);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            data-testid="student-sidebar-mobile-close-button"
          >
            <X size={20} />
          </button>
        </div>
        ...
        {navigation.map((item) => {
          ...
          return (
            <Link
              ...
              data-testid={`student-sidebar-nav-${item.id}`}
      ...
      <div className="border-t border-sidebar-border p-4 space-y-2 mt-auto">
        <button
          ...
          data-testid="student-sidebar-language-toggle"
        >
        ...
        <button
          ...
          className={`group flex w-full items-center px-2 py-2 text-sm font-medium rounded-md text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground ${isCollapsed ? 'lg:justify-center' : ''}`}
          data-testid="student-sidebar-theme-toggle"
        >
          {theme === 'dark' ? (
        ...
        <button
          ...
          data-testid="student-sidebar-logout-button"
        >
          <LogOut
            className={`h-5 w-5 shrink-0 me-3 ${isCollapsed ? "lg:me-0" : ""}`}
          />
