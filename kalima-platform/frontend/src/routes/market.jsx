...
export const getProductsBySection = async (sectionId, queryParams = {}) => {
  try {
    const response = await axios.get(
      `${API_URL}/ec/sections/${sectionId}/products`,
      {
        params: queryParams,
        withCredentials: true,
        headers: {
...
export const createProduct = async (productData) => {
  try {
    ...
    if (
      ...
      productData.subSection === "" ||
      productData.subSection === "undefined"
    ) {
      console.error("❌ DEBUG createProduct - SubSection validation failed!");
      throw new Error(
        "SubSection is required. Please select a subsection before creating the product.",
      );
    }
    ...
    if (productData.thumbnail) {
      ...
      console.log(
        "[createProduct] Appending thumbnail:",
    ...
    if (productData.sample) {
      ...
      console.log(
        "[createProduct] Appending sample:",
        productData.sample.name,
        "=>",
        safeSample,
      );
    ...
    if (productData.gallery && productData.gallery.length > 0) {
      ...
      for (let i = 0; i < productData.gallery.length; i++) {
        ...
        console.log(
          `[createProduct] Appending gallery[${i}]:`,
          productData.gallery[i].name,
          "=>",
          safeGallery,
        );
...
export const createBook = async (bookData) => {
  try {
    ...
    if (
      !bookData.subSection ||
      bookData.subSection === "" ||
      bookData.subSection === "undefined"
    ) {
      console.error("❌ DEBUG createBook - SubSection validation failed!");
      throw new Error(
        "SubSection is required. Please select a subsection before creating the book.",
      );
    }
    ...
    if (bookData.thumbnail) {
      ...
      console.log(
        "[createBook] Appending thumbnail:",
        bookData.thumbnail.name,
        "=>",
        safeThumb,
      );
    ...
    if (bookData.sample) {
      ...
      console.log(
        "[createBook] Appending sample:",
        bookData.sample.name,
        "=>",
        safeSample,
      );
    ...
    if (bookData.gallery && bookData.gallery.length > 0) {
      ...
      for (let i = 0; i < bookData.gallery.length; i++) {
        ...
        console.log(
          `[createBook] Appending gallery[${i}]:`,
          bookData.gallery[i].name,
          "=>",
          safeGallery,
        );
...
export const updateProduct = async (productId, productData) => {
  try {
    ...
    if (productData.title !== undefined)
      formData.append("title", productData.title);
    if (productData.serial !== undefined)
      formData.append("serial", productData.serial);
    if (productData.section !== undefined)
      formData.append("section", productData.section);
    if (productData.subSection !== undefined)
      formData.append("subSection", productData.subSection);
    if (productData.price !== undefined)
      formData.append("price", productData.price);
    if (productData.priceAfterDiscount !== undefined)
      formData.append("priceAfterDiscount", productData.priceAfterDiscount);
    if (productData.paymentNumber !== undefined)
      formData.append("paymentNumber", productData.paymentNumber);
    if (productData.whatsAppNumber !== undefined)
      formData.append("whatsAppNumber", productData.whatsAppNumber);
    if (productData.subject !== undefined)
      formData.append("subject", productData.subject);
    ...
    if (productData.thumbnail) {
      const safeThumb = generateSafeFilename(productData.thumbnail);
      console.log(
        "[updateProduct] Appending thumbnail:",
        productData.thumbnail.name,
        "=>",
        safeThumb,
      );
    ...
    if (productData.sample) {
      const safeSample = generateSafeFilename(productData.sample);
      console.log(
        "[updateProduct] Appending sample:",
        productData.sample.name,
        "=>",
        safeSample,
      );
...
export const purchaseProduct = async (purchaseData) => {
  try {
    ...
    formData.append(
      ...
      purchaseData.numberTransferredFrom,
    );
    ...
    if (purchaseData.paymentScreenShot) {
      ...
      formData.append(
        "paymentScreenShot",
        purchaseData.paymentScreenShot,
        safePayment,
      );
    }
...
export const getSubSectionsBySection = async (sectionId, queryParams = {}) => {
  try {
    const response = await axios.get(
      `${API_URL}/ec/sections/${sectionId}/subsections`,
      {
        params: queryParams,
        withCredentials: true,
        headers: {
...
export const getSubSectionById = async (subSectionId) => {
  try {
    const response = await axios.get(
      `${API_URL}/ec/subsections/${subSectionId}`,
      {
        withCredentials: true,
        headers: {
          Authorization: `Bearer ${getToken()}`,
