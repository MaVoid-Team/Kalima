...
const LectureDisplay = () => {
  ...
  const getFileIcon = (fileType) => {
    ...
    if (type.includes("pdf")) {
      return <FiFile className="text-error text-xl" />;
    } else if (
      type.includes("image") ||
      type.includes("png") ||
      type.includes("jpg") ||
      ...
      return <FiFile className="text-success text-xl" />;
    } else if (type.includes("video")) {
      return <FiFile className="text-info text-xl" />;
    } else if (type.includes("audio")) {
      return <FiFile className="text-secondary text-xl" />;
    } else {
        ...
        <div className="mb-6 shadow-xl rounded-lg overflow-hidden bg-base-300" ref={videoContainerRef}>
          ...
          <div className="p-4 bg-base-200">
            <div className="flex flex-col space-y-2">
              ...
              <div
                className="relative w-full h-4 bg-base-300 rounded-full cursor-pointer overflow-hidden group"
                ref={progressBarRef}
                onClick={handleProgressBarClick}
              >
                {/* Buffered progress */}
                <div
                  className="absolute top-0 left-0 h-full bg-base-content/30 opacity-50 transition-all duration-300"
                ...
                {/* Hover effect */}
                <div className="absolute top-0 left-0 w-full h-full opacity-0 group-hover:opacity-20 bg-base-content"></div>

                {/* Thumb indicator */}
                <div
                  className="absolute top-0 h-full aspect-square rounded-full bg-primary-focus border-2 border-base-100 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity"
                  style={{ left: `${progress}%` }}
                ></div>
              </div>
