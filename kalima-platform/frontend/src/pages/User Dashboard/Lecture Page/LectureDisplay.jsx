...
const LectureDisplay = () => {
  ...
  const [isLectureCreator, setIsLectureCreator] = useState(false)
    ...
    const fetchLecture = async () => {
      try {
        ...
        if (result.success) {
          ...
          // Check if the current user is the creator of this lecture
          if (userId && lectureData.createdBy?._id === userId) {
            console.log("User is the creator of this lecture")
            setIsLectureCreator(true)
          }
        } else {
  ...
  useEffect(() => {
    const fetchAccessData = async () => {
      // Skip this check for lecturers and admins
    ...
    // Only fetch access data if we know the user role and it's a student
  ...
  useEffect(() => {
    ...
    const initializePlayer = () => {
      try {
        ...
        // For lecturers and admins, don't block video regardless of view count
        const shouldBlockVideo = userRole === "Student" && (videoBlocked || remainingViews <= 0)

        if (shouldBlockVideo) return
        ...
        // View deduction logic - only for students
        const handleFirstPlay = async () => {
          // Skip view deduction for lecturers and admins
        ...
        if (savedTime) {
          player.ready(() => {
            player.currentTime(Number.parseFloat(savedTime))
          })
        }
      } catch (err) {
  ...
  }, [lecture?.videoLink, videoBlocked, remainingViews, studentLectureAccessId, lectureId, navigate, userRole])
            ...
            <li key={attachment._id} className="mt-2">
              <div className="flex gap-4 items-center">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleDownload(attachment._id)} className="text-green-600 hover:underline">
                    تحميل
                  </button>
                </div>
              </div>
  ...
  // Determine if video should be blocked based on user role
  const isVideoBlocked = userRole === "Student" && videoBlocked
    ...
    <div className="container mx-auto p-4" dir="rtl">
      ...
      {userRole && userRole !== "Student" && (
        <div className="alert alert-info mb-6">
          ...
          <span>أنت تشاهد هذه المحاضرة بصفتك {userRole === "Lecturer" ? "محاضر" : "مسؤول"}</span>
        </div>
      )}
      ...
      {isVideoBlocked ? (
        ...
        <div className="mb-6">
          ...
          ></video>
          <div className="mt-2">
            {userRole === "Student" && <p className="mt-2">المشاهدات المتبقية: {remainingViews}</p>}
          </div>
        </div>
      ) : (
      ...
      <div className="card bg-base-100 shadow-xl mb-6">
        <div className="card-body">
          ...
          <p>
            <strong>الوصف:</strong> {lecture.description || "لا يوجد وصف"}
          </p>
          <p>
            <strong>السعر:</strong> {lecture.price} نقاط
          </p>
          <p>
            <strong>النوع:</strong> {lecture.lecture_type}
          </p>
          <p>
            <strong>عدد المشاهدات الكلية:</strong> {lecture.numberOfViews}
          </p>
          <p>
          ...
          {lecture.requiresExam && (
            <p>
              <strong>يتطلب امتحان:</strong> نعم
            </p>
          )}
        </div>
      </div>
      ...
      <div className="card bg-base-100 shadow-xl">
        <div className="card-body">
          ...
          {attachments.exams.length === 0 &&
            ...
            attachments.pdfsandimages.length === 0 && <p>لا توجد مرفقات متاحة.</p>}
        </div>
      </div>
    </div>
  )
