...
function Register() {
  ...
  const handleRoleSpecificChange = useCallback((e) => {
    const { name, value } = e.target
  ...
  const renderForm = useMemo(() => (
    ...
    >
      ...
      <div className="form-control mt-6">
        <button
          className={`btn bg-button-background text-button-foreground border-none hover:bg-accent hover:text-foreground ${loading ? "loading" : ""}`}
          disabled={loading}
        >
  ...
  ), [renderCommonFields, renderTeacherFields, renderStudentFields, renderParentFields, activeTab, error, loading, handleSubmit])
      ...
      <div className="card flex-shrink-0 w-full max-w-md shadow-2xl bg-background">
        <div className="card-body">
          <div className="tabs tabs-boxed mb-4">
            ...
            <Link
              className={`tab ${activeTab === "student" ? "bg-primary duration-500 transition-all" : "text-foreground"}`}
              onClick={() => setActiveTab("student")}
            >
              Student
            </Link>
            <Link
              className={`tab ${activeTab === "teacher" ? "bg-primary duration-500 transition-all" : "text-foreground"}`}
