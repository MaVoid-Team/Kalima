...
export default function AdminEBookletTermsMilestonesPage() {
  ...
  useEffect(() => {
    if (!milestoneForm.termId && selectedTerm?.id) {
      setMilestoneForm((current) => ({ ...current, termId: String(selectedTerm.id) }));
    }
  }, [milestoneForm.termId, selectedTerm?.id]);
  ...
  return (
    <div className="space-y-6" data-testid="admin-e-booklet-terms-milestones-page">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-3xl font-bold tracking-tight">
            <CalendarRange className="h-8 w-8 text-primary" />
            {t("admin.termsMilestones.title")}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">{t("admin.termsMilestones.description")}</p>
        </div>
      ...
      <div className="grid gap-4 rounded-lg border bg-background p-4 md:grid-cols-3">
        <div>
          <div className="text-sm text-muted-foreground">{t("admin.termsMilestones.activeTerms")}</div>
          <div className="mt-1 text-2xl font-semibold">{activeTerms.length}</div>
        </div>
        <div>
          <div className="text-sm text-muted-foreground">{t("admin.termsMilestones.milestoneCount")}</div>
          <div className="mt-1 text-2xl font-semibold">{scopedMilestones.length}</div>
        </div>
        <div>
          <div className="text-sm text-muted-foreground">{t("admin.termsMilestones.paidRedemptions")}</div>
          <div className="mt-1 text-2xl font-semibold">{progress?.paidRedemptions ?? 0}</div>
        </div>
      </div>
              ...
              <TableBody>
                {scopedMilestones.map((milestone) => (
                  <TableRow key={milestone.id}>
                    <TableCell><div className="font-medium">{milestone.title}</div><div className="text-xs text-muted-foreground">#{milestone.sort_order ?? 0} · {milestone.description || "—"}</div></TableCell>
      ...
      <section className="rounded-lg border bg-background p-4" data-testid="admin-e-booklet-progress-view">
        <div className="mb-3 flex items-center gap-2"><Medal className="h-5 w-5 text-primary" /><h2 className="font-semibold">{t("admin.termsMilestones.progressTitle")}</h2></div>
        <div className="grid gap-3 md:grid-cols-3">
          <div className="rounded-md border p-3"><div className="text-sm text-muted-foreground">{t("admin.termsMilestones.paidRedemptions")}</div><div className="text-2xl font-semibold">{progress?.paidRedemptions ?? 0}</div></div>
          <div className="rounded-md border p-3"><div className="text-sm text-muted-foreground">{t("admin.termsMilestones.achievementCount")}</div><div className="text-2xl font-semibold">{progress?.achievements?.length ?? 0}</div></div>
          <div className="rounded-md border p-3"><div className="text-sm text-muted-foreground">{t("admin.termsMilestones.claimedCount")}</div><div className="text-2xl font-semibold">{(progress?.achievements || []).filter((item) => item.claimed_at).length}</div></div>
        </div>
        <div className="mt-4 space-y-2">
          {(progress?.teacherProgress || []).slice(0, 8).map((row) => (
            <div key={row.teacherId} className="flex flex-wrap items-center justify-between gap-2 rounded-md border p-3 text-sm">
