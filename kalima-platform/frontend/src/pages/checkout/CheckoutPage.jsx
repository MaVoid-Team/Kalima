...
export default function CheckoutPage() {
    const { cart, loading, checkout, getPaymentMethods } = useCart();
    const { t, i18n } = useTranslation('checkout');
    const navigate = useNavigate();
    // derive origin like cart page
    ...
    const handlePay = async () => {
        ...
        // capture method name for receipt (payment methods may be simple list)
        try {
        ...
        try {
            ...
            // optionally clear cart or navigate later
        } catch (e) {
            toast.error(t('failed') || 'Checkout failed, please try again', { description: e.message });
        }
    };

    // transform cart items to order summary structure
    ...
    const pricing = {
        ...
        const handlePrintReceipt = () => {
            ...
            // copy stylesheets so receipt classes render correctly
            const styleNodes = document.querySelectorAll('style, link[rel="stylesheet"]');
            styleNodes.forEach((node) => {
                printWindow.document.head.appendChild(node.cloneNode(true));
            });

            // add base print styles and append cloned receipt node
    ...
    return (
        <div className="relative min-h-screen bg-background flex flex-col overflow-x-hidden">
            ...
            <main className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-8 max-w-6xl mx-auto px-4 md:px-8 py-8 flex-1 overflow-x-hidden">
            ...
            {/* printable-only copy of the receipt - visible only during printing */}
