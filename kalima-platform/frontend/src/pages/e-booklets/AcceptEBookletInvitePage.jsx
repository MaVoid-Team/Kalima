...
import { BookOpenCheck, CreditCard, KeyRound, Loader2, Ticket, Upload } from "lucide-react";
...
export default function AcceptEBookletInvitePage({ mode = "invite" }) {
  ...
  useEffect(() => {
    if (!isAuthenticated || isCodeMode) return;
    let active = true;
  ...
  const submit = async (accessPath) => {
    ...
    try {
      ...
      if (accessPath === "online_purchase") {
        ...
        payload = { accessPath, termsAccepted: true, termsVersion: TERMS_VERSION };
      ...
      {isCodeMode ? (
        ...
        <>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <section className="space-y-3 rounded-lg border bg-background p-4">
              <h2 className="flex items-center gap-2 font-semibold"><CreditCard className="h-4 w-4" />{t("inviteAccept.purchaseTitle")}</h2>
              <p className="text-sm text-muted-foreground">{t("inviteAccept.purchaseDescription")}</p>
              <div className="rounded-md bg-muted p-2 text-xs text-muted-foreground">{t("inviteAccept.internalPriceHidden")}</div>
              <Label>{t("inviteAccept.paymentMethod", { defaultValue: "Payment method" })}</Label>
              <select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={paymentMethodId} onChange={(event) => setPaymentMethodId(event.target.value)}>
                <option value="">{t("inviteAccept.selectPaymentMethod", { defaultValue: "Select payment method" })}</option>
              ...
              <Label>{t("inviteAccept.numberTransferredFrom", { defaultValue: "Number transferred from" })}</Label>
              <Input value={numberTransferredFrom} onChange={(event) => setNumberTransferredFrom(event.target.value)} placeholder={t("inviteAccept.numberTransferredFromPlaceholder", { defaultValue: "Your wallet/mobile number" })} />
              <Label>{t("inviteAccept.paymentScreenshot", { defaultValue: "Payment proof screenshot" })}</Label>
              <Input type="file" accept="image/*" onChange={(event) => setPaymentScreenshot(event.target.files?.[0] || null)} />
              <Textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder={t("inviteAccept.notesPlaceholder", { defaultValue: "Optional notes for the admin" })} />
              <Button className="w-full" variant="outline" onClick={() => submit("online_purchase")} disabled={state.status === "loading" || !paymentMethodId || !numberTransferredFrom || !paymentScreenshot}>{state.status === "loading" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}{t("inviteAccept.requestOnlinePurchase")}</Button>
