...
class ZonesService {
  ...
  async createZone(dto: CreateZoneDto): Promise<zones> {
    ...
    if (existing)
      throw new ConflictError(
        "Zone with this title already exists for the government",
      );
  ...
  async getAllZones(filters?: {
    ...
    if (filters?.government_id !== undefined)
      where.government_id = filters.government_id;
    if (filters?.active !== undefined) where.active = filters.active;

    const list = await this.db.zones.findMany({
      where,
      orderBy: { title: "asc" },
    });
    return list;
  ...
  async updateZone(id: number, dto: UpdateZoneDto): Promise<zones> {
    ...
    if (newTitle !== z.title || newGovId !== z.government_id) {
      const existing = await this.db.zones.findFirst({
        where: { title: newTitle, government_id: newGovId },
      });
  ...
  async deleteZone(id: number): Promise<void> {
    ...
    if (parentsCount || studentsCount || teachersCount) {
      throw new BadRequestError(
        "Cannot delete zone while related parents/students/teachers exist",
      );
    }
