...
import { government } from "../generated/prisma/client";
import {
  BadRequestError,
  NotFoundError,
  ConflictError,
...
class GovernmentService {
  ...
  async createGovernment(dto: CreateGovernmentDto): Promise<government> {
    ...
    if (existing)
      throw new ConflictError("Government with this title already exists");
  ...
  async getAllGovernments(filters?: {
    ...
    const list = await this.db.government.findMany({
      where,
      orderBy: { title: "asc" },
    });
    return list;
  ...
  async deleteGovernment(id: number): Promise<void> {
    ...
    const [zonesCount, teachersCount, studentsCount, parentsCount] =
      await Promise.all([
