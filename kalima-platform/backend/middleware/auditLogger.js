...
const createAuditLogEntry = async (req, res, originalRes) => {
  try {
    ...
    if (!req.user) {
      console.log('[AUDIT] Skipped: No user in request');
      return;
    }
    ...
    console.log('[AUDIT] URL:', req.originalUrl, 'Method:', req.method, 'PathSegments:', pathSegments);
    ...
    const resourceIdentifiers = [
      ...
      { keywords: ["cart-purchases", "cartpurchases"], type: "ec.cartpurchase" },
      { keywords: ["book-purchases", "bookpurchases"], type: "ec.bookPurchase" },
      { keywords: ["lessons"], type: "lesson" },
      { keywords: ["subsections"], type: "ec.subSection" },
      { keywords: ["timetable", "timetables"], type: "timetable" },
      { keywords: ["subject", "subjects"], type: "subject" },
      { keywords: ["level", "levels"], type: "level" },
      { keywords: ["payment-methods", "paymentmethods"], type: "PaymentMethod" },
      { keywords: ["whatsapp-numbers", "whatsappnumbers"], type: "whatsAppNumber" }
    ];
    ...
    for (const resource of resourceIdentifiers) {
      ...
      if (resource.type.startsWith("ec.") && pathSegments.includes("ec")) {
        if (resource.keywords.some(keyword => {
          // Check if keyword matches any segment directly (for hyphenated keywords like "cart-purchases")
          if (pathSegments.includes(keyword)) return true;
          // Also check case-insensitive match
          return pathSegments.some(segment => segment.toLowerCase() === keyword.toLowerCase());
        })) {
      ...
      else if (!resource.type.startsWith("ec.")) {
        if (resource.keywords.some(keyword => {
    ...
    if (!resourceType) {
      console.log('[AUDIT] Skipped: Resource type not recognized for URL:', req.originalUrl);
      return;
    }

    console.log('[AUDIT] Resource type detected:', resourceType);
    ...
    if (resourceType === "ec.purchase") {
      ...
      // For any other PATCH/PUT operations on purchases, still log them
      else if ((req.method === "PATCH" || req.method === "PUT") && req.params.id) {
        specialAction = "update";
        // Will get resource name from response data below
      }
    }

    // Special case for cart purchase operations
    if (resourceType === "ec.cartpurchase") {
      // For any PATCH/PUT operations on cart purchases, log them
    ...
    if (resData && !specialResource) {
      ...
      else if (resourceType === "code" && resData.code) {
        resourceId = resourceId || resData.code._id || resData.code.id;
        resourceName = resData.code.codeValue || resData.code.code;
      }
      else if (resourceType === "level" && resData.level) {
        resourceId = resourceId || resData.level._id;
        resourceName = resData.level.name;
      }
      else if (resourceType === "subject" && resData.subject) {
        resourceId = resourceId || resData.subject._id;
        resourceName = resData.subject.name;
      }
      ...
      else if (resourceType === "ec.cartPurchase" && resData.cartPurchase) { // Handle e-commerce cart purchase responses
        resourceId = resourceId || resData.cartPurchase._id;
        resourceName = `Cart Purchase ${resData.cartPurchase.purchaseSerial || resData.cartPurchase._id}`;
      }
      else if (resourceType === "PaymentMethod" && resData.paymentMethod) { // Handle payment method responses
        resourceId = resourceId || resData.paymentMethod._id;
        resourceName = resData.paymentMethod.name || `Payment Method ${resData.paymentMethod._id}`;
      }
      else if (resourceType === "whatsAppNumber" && resData.whatsAppNumber) { // Handle e-commerce subsection responses
        resourceId = resourceId || resData.whatsAppNumber._id;
        resourceName = resData.whatsAppNumber.number || `WhatsApp Number ${resData.whatsAppNumber._id}`;
      }
    ...
    const isSuccess = statusCode >= 200 && statusCode < 400 && (
      ...
      resourceId ||
      (resData && (resData.results > 0 || resData.message || resData.package || resData.packages || resData.lesson || resData.timetable || resData.attendance || resData.totalRevenue !== undefined || resData.breakdown || resData.section || resData.product || resData.purchase || resData.cartPurchase))
    );
    ...
    console.log('[AUDIT] Creating log entry:', {
      resourceType,
      resourceId,
      action: finalAction,
    ...
    console.error('[AUDIT] Error in createAuditLogEntry:', error.message, error.stack);
  }
};
