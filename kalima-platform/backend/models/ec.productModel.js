...
function setPriceAfterDiscount(next) {
    ...
    const discount = typeof this.discountPercentage === 'number' ? this.discountPercentage : 0;
...
productSchema.pre("findOneAndUpdate", function (next) {
    ...
    if (update.$set) {
        ...
        discount = update.$set.discountPercentage !== undefined ? update.$set.discountPercentage : discount;
    }
    // Fallback to current doc if not provided
    this.model.findOne(this.getQuery()).then(doc => {
        const finalPrice = typeof price === 'number' ? price : (doc ? doc.price : 0);
        const finalDiscount = typeof discount === 'number' ? discount : (doc ? doc.discountPercentage : 0);
