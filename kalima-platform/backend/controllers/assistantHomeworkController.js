// DOMAIN: ACADEMY
// STATUS: LEGACY
// NOTE: Academy assistant homework logic.
...
exports.getLecturesWithHomework = catchAsync(async (req, res, next) => {
  ...
  if (!assistant || !assistant.assignedLecturer) {
    return next(
      new AppError("Assistant not found or not assigned to a lecturer", 404),
    );
  }
  ...
  const filteredLectures =
    req.query.onlyWithSubmissions === "true"
...
exports.getLectureHomeworkSubmissions = catchAsync(async (req, res, next) => {
  ...
  if (req.user.role === "Lecturer") {
    ...
    if (lecture.createdBy._id.toString() !== req.user._id.toString()) {
      return next(
        new AppError("You don't have permission to access this lecture", 403),
      );
    }
    ...
    const assistant = await Assistant.findById(req.user._id).populate(
      "assignedLecturer",
    );
    ...
    if (
      ...
      assistant.assignedLecturer._id.toString()
    ) {
      return next(
        new AppError("You don't have permission to access this lecture", 403),
      );
