// DOMAIN: STORE
// STATUS: LEGACY
// NOTE: Store referral program logic.
  ...
  try {
    ...
    for (const user of referredUsers) {
      ...
      if (purchaseCount > 0) {
        ...
        if (!inviterToInvitees[inviterId])
          inviterToInvitees[inviterId] = new Set();
    ...
    for (const inviterId in inviterToInvitees) {
      ...
      if (Model) {
        allUpdates.push(
          Model.findByIdAndUpdate(inviterId, {
            successfulInvites: inviteCount,
          }),
        );
      }
    }
    ...
    res
      ...
      .json({
        status: "success",
        message: "All successfulInvites counters recalculated.",
      });
  } catch (err) {
    next(err);
  }
...
exports.getECReferralStatsByUser = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ status: "success", data: invitedWithECPurchase[0] || null });
  } catch (err) {
    next(err);
  }
};
...
exports.recalculateInviterSuccessfulInvites = async (inviterId) => {
  ...
  if (Model) {
    await Model.findByIdAndUpdate(inviterId, {
      successfulInvites: inviteCount,
    });
  }
};
