// DOMAIN: ACADEMY
// STATUS: LEGACY
// NOTE: Academy lesson management logic.
...
const validateRelatedDocs = async (
  ...
  const [subject, lecturer, level, center, groupedLesson] = await Promise.all([
    ...
    Center.findById(centerId, null, findOptions), // Validate groupedLessons document only if courseOrmonthId is provided
    courseOrmonthId
      ? groupedLessonsModel.findById(courseOrmonthId, null, findOptions)
  ...
  if (courseOrmonthId && !groupedLesson) {
    throw new AppError(
      "Related groupedLessons document not found with the provided ID.",
      404,
    );
...
exports.createLesson = catchAsync(async (req, res, next) => {
  ...
  const {
    ...
    center,
    courseOrmonth,
  } = req.body; // 2. Modify validation to exclude courseOrmonth from strictly required fields if it's optional in the schema (which it is)
  ...
  try {
    ...
    const resultData = await session.withTransaction(async () => {
      ...
      if (courseOrmonth) {
        updatedGroupedLesson = await groupedLessonsModel.findByIdAndUpdate(
          // Using the imported model variable
...
exports.updateLesson = catchAsync(async (req, res, next) => {
  ...
  if (
    updateData.courseOrmonth !== undefined &&
    updateData.courseOrmonth !== null &&
    !mongoose.Types.ObjectId.isValid(updateData.courseOrmonth)
  ) {
    return next(
      new AppError("Invalid courseOrmonth ID format in update data.", 400),
    );
  }
  ...
  try {
    ...
    const updatedLesson = await session.withTransaction(async () => {
      ...
      await validateRelatedDocs(
        ...
        updateData.courseOrmonth !== undefined
          ? updateData.courseOrmonth
          : currentLesson.courseOrmonth, // Handle explicit removal (setting to null/undefined)
        session, // Pass session
      );

      // Handle potential update of courseOrmonth and its inverse relationship
      const oldCourseOrmonthId = currentLesson.courseOrmonth
        ? currentLesson.courseOrmonth.toString()
        : null;
      ...
      if (
        oldCourseOrmonthId &&
        (!newCourseOrmonthId ||
          newCourseOrmonthId.toString() !== oldCourseOrmonthId)
      ) {
