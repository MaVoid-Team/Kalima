...
exports.createStudentLectureAccess = catchAsync(async (req, res, next) => {
  ...
  if (lectureDoc.requiresExam) {
    ...
    if (!examSubmission) {
      return next(new AppError("You must pass the exam before accessing this lecture", 403));
    }
  }
    ...
    if (!examSubmission) {
      return res.status(403).json({
        ...
        data: {
          ...
          passingThreshold: lecture.passingThreshold || 
                           (lecture.examConfig ? lecture.examConfig.defaultPassingThreshold : 60)
        }
      });
    }
  }
