// DOMAIN: ACADEMY
// STATUS: LEGACY
// NOTE: Academy grouped lessons logic.
...
exports.updateCourse = catchAsync(async (req, res, next) => {
  ...
  if (Object.keys(updates).length === 0) {
    return next(
      new AppError(
        "No valid fields provided for update (only name and description are directly updateable).",
        400,
      ),
    );
  }
...
exports.deleteCourse = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (lessonIdsToDelete && lessonIdsToDelete.length > 0) {
      await Lesson.deleteMany({ _id: { $in: lessonIdsToDelete } }, { session });
    }
