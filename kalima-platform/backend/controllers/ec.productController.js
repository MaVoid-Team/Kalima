// DOMAIN: STORE
// STATUS: LEGACY
// NOTE: Store product controller logic.
...
exports.getAllProducts = async (req, res) => {
  try {
    const products = await ECProduct.find()
      .populate({
        path: "section",
        select: "number",
      })
...
exports.getProductById = async (req, res) => {
  try {
    const product = await ECProduct.findById(req.params.id)
      .populate({
        path: "section",
        select: "number",
      })
  ...
  try {
    ...
    const finalPriceAfterDiscount =
      updateData.priceAfterDiscount !== undefined
        ...
        : existingProduct.priceAfterDiscount;
    ...
    if (req.files?.sample?.[0]) {
      ...
      if (file.size > 150 * 1024 * 1024) {
        return res
          .status(400)
          .json({ message: "Sample file size must be <= 150MB" });
      }
    ...
    if (req.files?.thumbnail?.[0]) {
      ...
      if (!allowedImageTypes.includes(file.mimetype)) {
        return res
          .status(400)
          .json({ message: "Thumbnail must be JPEG, PNG, or WebP" });
      }
      if (file.size > 5 * 1024 * 1024) {
        return res
          .status(400)
          .json({ message: "Thumbnail file size must be <= 5MB" });
      }
      if (
    ...
    if (req.files?.gallery?.length > 0) {
      ...
      for (const file of req.files.gallery) {
        if (!allowedImageTypes.includes(file.mimetype)) {
          return res
            .status(400)
            .json({ message: "All gallery images must be JPEG, PNG, or WebP" });
        }
        if (file.size > 5 * 1024 * 1024) {
          return res
            .status(400)
            .json({ message: "Each gallery image must be <= 5MB" });
        }
      }
