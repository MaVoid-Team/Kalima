// DOMAIN: ACADEMY
// STATUS: LEGACY
// NOTE: Academy lecture management logic.
...
const parseBoolean = (value) => {
  ...
  if (typeof value === "string") {
    ...
    if (trimmed === "true" || trimmed === "1" || trimmed === "yes") {
      console.log(
        "[v0] parseBoolean string true result:",
        true,
        "from:",
        value,
      );
    ...
    if (
      ...
      console.log(
        "[v0] parseBoolean string false result:",
        false,
        "from:",
        value,
      );
...
exports.createLecture = catchAsync(async (req, res, next) => {
  ...
  uploadSingleImageToDisk(req, res, async (uploadErr) => {
    ...
    try {
      ...
      console.log(
        "[v0] Raw requiresHomework:",
        requiresHomework,
        "type:",
        typeof requiresHomework,
      );

      const parsedRequiresExam =
        requiresExam !== undefined ? parseBoolean(requiresExam) : false;
      const parsedRequiresHomework =
        requiresHomework !== undefined ? parseBoolean(requiresHomework) : false;
      const parsedTeacherAllowed =
        teacherAllowed !== undefined ? parseBoolean(teacherAllowed) : true;
      const parsedNumberOfViews =
      ...
      console.log(
        "[v0] Created lecture requiresHomework:",
        lecture[0].requiresHomework,
      );
      console.log(
        "[v0] Created lecture requiresExam:",
        lecture[0].requiresExam,
      );
...
exports.getAllLecturesPublic = catchAsync(async (req, res, next) => {
  ...
  query = query.select(
    "name type subject level createdBy price description lecture_type teacherAllowed thumbnail",
  );
...
exports.updatelectures = catchAsync(async (req, res, next) => {
  ...
  uploadSingleImageToDisk(req, res, async (uploadErr) => {
    ...
    session.startTransaction();
    try {
      const currentLecture = await Lecture.findById(
      ...
      const obj = {
        ...
        lecture_type,
        teacherAllowed:
          teacherAllowed !== undefined
            ? parseBoolean(teacherAllowed)
            : undefined,
      };
      ...
      try {
        ...
        if (!template) {
          ...
          if (studentIdsWithAccess.length > 0) {
            ...
            if (examConfig)
              updateDescription = "Exam requirements were updated";
            if (homeworkConfig)
