...
exports.createCartPurchase = catchAsync(async (req, res, next) => {
    ...
    if (
        hasBooks &&
        (!req.body.nameOnBook || !req.body.numberOnBook || !req.body.seriesName)
    ) {
        return next(
            new AppError(
    ...
    const items = cart.items.map((item) => ({
        ...
        numberOnBook:
            item.product.__t === "ECBook" ? req.body.numberOnBook : undefined,
...
exports.getAllPurchases = catchAsync(async (req, res, next) => {
    ...
    if (req.query.minTotal || req.query.maxTotal) {
        ...
        if (req.query.minTotal)
            baseMatch.total.$gte = parseFloat(req.query.minTotal);
        if (req.query.maxTotal)
            baseMatch.total.$lte = parseFloat(req.query.maxTotal);
    }
