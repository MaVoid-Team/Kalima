// DOMAIN: STORE
// STATUS: LEGACY
// NOTE: Store payment method management logic.
...
exports.createPaymentMethod = catchAsync(async (req, res, next) => {
  ...
  if (!name || !phoneNumber || (!req.file && !paymentMethodImg)) {
    return next(
      new AppError(
        "Name, phone number, and payment method image are required",
        400,
      ),
    );
  }
  ...
  if (existingMethod) {
    return next(
      new AppError("Payment method with this name already exists", 400),
    );
  }
...
exports.getPaymentMethodById = catchAsync(async (req, res, next) => {
  const paymentMethod = await PaymentMethod.findById(req.params.id).select(
    "-__v",
  );
...
exports.changePaymentMethodStatus = catchAsync(async (req, res, next) => {
  ...
  if (typeof status !== "boolean") {
    return next(
      new AppError("Status must be a boolean value (true/false)", 400),
    );
  }
