...
describe("EBookletService", () => {
  ...
  describe("admin write DTO persistence", () => {
    ...
    test("updateTemplate synchronizes instance display titles and branding with the new template title", async () => {
      const db = createMockDb();
      ...
      db.e_booklet_instances.findMany.mockResolvedValue([
        { id: 101, display_title: "Old Title", branding_json: null, purchase_id: 11 },
        { id: 102, display_title: "Old Title", branding_json: { bookletTitle: "Teacher Custom Title" }, purchase_id: null },
        { id: 103, display_title: "Teacher e-booklet #103", branding_json: {}, purchase_id: null },
      ]);
      db.e_booklet_purchases.findUnique.mockResolvedValue({ id: 11, branding_json: null });
      db.e_booklet_instances.update.mockResolvedValue({ id: 101 });
      db.e_booklet_purchases.update.mockResolvedValue({ id: 11 });
      ...
      expect(db.e_booklet_instances.update).toHaveBeenCalledWith({
        where: { id: 101 },
        data: expect.objectContaining({
          display_title: "Renamed Title",
          branding_json: { bookletTitle: "Renamed Title" },
        }),
      });
      expect(db.e_booklet_instances.update).toHaveBeenCalledWith({
        where: { id: 102 },
      ...
      expect(db.e_booklet_purchases.update).toHaveBeenCalledWith({
        where: { id: 11 },
        data: expect.objectContaining({
          branding_json: { bookletTitle: "Renamed Title" },
        }),
